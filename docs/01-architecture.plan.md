...
### 검증 항목
- [x] 타일 Prefab 8종이 Assets/Prefabs/Tiles/에 존재
- [x] 파티클 Prefab 4종이 Assets/Prefabs/Effects/에 존재 (MatchBurst, MatchStar, MatchGlow, MatchRing)
- [x] UI Prefab 4종이 Assets/Prefabs/UI/에 존재 (LevelButton, HudPanel, GameOverPanel, BoosterButton)
- [x] GameConfig.asset이 Assets/ScriptableObjects/에 존재
- [x] LevelConfig.asset × 5개가 Assets/ScriptableObjects/Levels/에 존재
- [x] Sprite Atlas 2종 생성 (TileAtlas, UIAtlas)
- [x] TileView.cs 스크립트 생성
- [x] 기존 게임 플레이에 영향 없음
- [x] 씬 배경 스프라이트 직접 할당 (Level01~05) — Edit 모드 배경 보임

**Phase 1 완료!** → 다음: Phase 2 (Model 분리)

---

## Phase 2: Model 분리 (순수 C# — Unity 의존 없음)
**위험도: 중간** — 핵심 로직 분리, GameGrid에서 호출만 변경
**상태: 미착수 (Phase 1 완료 후 진행)**
